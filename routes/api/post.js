    ...
    Post.findOne({ _id: req.params.id })
        .then(post => {
            ...
            if (post.user.toString() === req.user.id) {
                ...
                errors.post = "Unable to delete post: Unauthorized access to someone's post";
                return res.json(errors);
            }
        })
        .catch(err => {
...
Router.post('/comment/:post_id', passport.authenticate('jwt', { session: false }), (req, res) => {
    ...
    Post.findOne({ _id: req.params.post_id })
        .then(post => {
            ...
            post.save().then(post => res.json(post))
                .catch(err => res.status(500).json({ saveError: 'Unable to save post' }));
        ...
        .then(post => {
            ...
            if (post.comments.filter((comment) => comment._id.toString() === req.params.commentId).length > 0) {
                ...
                if (post.comments[removeIndex].user.toString() === req.user.id) {
                    ...
                    post.save().then(post => res.json(post)).catch(err => res.json({ comment: 'Error deleting comment' }));
